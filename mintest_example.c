...
/* 待测试函数1，返回两个int型数字和全局变量test_int_value的和，并更新test_int_value的值加1 */
...
/* 待测试函数2，返回两个double型数字和全局变量test_int_value的和，并更新test_int_value的值加1 */
...
MT_TESTCASE(test_assert_fail)
{
    ...
    mt_assert(my_add_int(1, 1) == 3, "my_add_int(1, 1) calc result should be 5"); /* FAIL */
}
...
MT_TESTCASE(test_assert_string_eq_fail)
{
    ...
    mt_assert_string_eq("a test string1", test_string); /* FAIL */
    mt_assert_string_eq(NULL, test_string);             /* FAIL */
}

/* 定义测试用例test_fail_message_exceeds_max_num，验证测试失败消息数超过缓存最大值的情况 */
MT_TESTCASE(test_fail_message_exceeds_max_num)
{
    /* 默认每个用例只缓存输出8条消息，这里最后两条消息会被截断 */
    mt_assert_int_eq(2, my_add_int(1, 1)); /* FAIL */
    mt_assert_int_eq(2, my_add_int(1, 1)); /* FAIL */
    mt_assert_int_eq(2, my_add_int(1, 1)); /* FAIL */
    mt_assert_int_eq(2, my_add_int(1, 1)); /* FAIL */
    mt_assert_int_eq(2, my_add_int(1, 1)); /* FAIL */
...
MT_TESTSUITE(test_suite1)
{
    ...
    MT_RUN_TESTCASE(test_fail_message_exceeds_max_num);
}
...
MT_TESTSUITE(test_suite2)
{
    ...
    MT_RUN_TESTCASE(test_assert_int_eq_pass); /* 预期全部成功，因为test_int_value初始值为1 */
    MT_RUN_TESTCASE(test_assert_int_eq_pass); /* 预期全部失败，因为test_int_value初始值为4 */
}
